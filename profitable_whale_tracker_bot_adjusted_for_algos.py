#!/usr/bin/env python3
"""
Whale 17 Position Tracker V2 - RAILWAY VERSION (STRICT BTC/ETH/SOL ONLY)
- STRICT FILTERING: Only tracks BTC, ETH, and SOL positions
- Ignores all other tokens including numbered spot positions (@142, etc)
...
class WhaleTracker:
    """
    Whale Position Tracker V2 - Railway Version (BTC/ETH/SOL ONLY)
    """
    def __init__(self):
        ...
        # WHITELIST - Only these tokens will trigger notifications
        ...
        print("WHALE POSITION TRACKER V2 - BTC/ETH/SOL ONLY")
        ...
        print(f"Notifications: BTC, ETH, SOL only")
    ...
    def test_telegram(self):
        ...
        try:
            ...
            self.send_telegram_message(
                ...
                f"🎯 <b>Notifications: BTC, ETH, SOL only</b>\n"
    ...
    def load_positions(self):
        ...
        if os.path.exists(positions_file):
            try:
                ...
                # STRICT FILTER: Only load whitelisted positions
                self.positions = {}
                filtered_count = 0
                for coin, pos in all_positions.items():
                    if coin in self.WHITELISTED_TOKENS:
                        self.positions[coin] = pos
                    else:
                        filtered_count += 1
                
                if self.positions:
                    print(f"Loaded {len(self.positions)} BTC/ETH/SOL positions")
                    for coin, pos in self.positions.items():
                        print(f"  {coin}: ${pos['value']:,.0f} ({pos['side']})")
                
                if filtered_count > 0:
                    print(f"Filtered out {filtered_count} non-whitelisted positions")
                    self.save_positions()  # Save cleaned data
    ...
    async def verify_all_positions(self):
        ...
        # Clean up non-whitelisted positions from saved data
        coins_to_remove = []
        for coin in self.positions.keys():
            if coin not in self.WHITELISTED_TOKENS:
                coins_to_remove.append(coin)
                positions_cleaned += 1
        
        for coin in coins_to_remove:
            print(f"   🧹 Removing non-whitelisted position: {coin}")
            del self.positions[coin]
        
        if positions_cleaned > 0:
            print(f"   🧹 Cleaned {positions_cleaned} non-whitelisted positions")
            self.save_positions()
        ...
        for coin in coins_to_verify_close:
            ...
            if is_closed:
                notification_status = "📢" if coin in self.WHITELISTED_TOKENS else "🔇"
                print(f"\n🚨 CONFIRMED CLOSED POSITION: {coin} {notification_status}")
                ...
                # Only send notification if whitelisted
                if coin in self.WHITELISTED_TOKENS:
    ...
    async def process_aggregated_fills(self, coin, side):
        ...
        # STRICT FILTER: Double-check whitelist
        if coin not in self.WHITELISTED_TOKENS:
            return
        ...
        notification_status = "📢" if coin in self.WHITELISTED_TOKENS else "🔇"
        print(f"\n📊 Aggregated {num_fills} fills: {coin} {notification_status}")
        ...
        # Only send notifications for whitelisted tokens
        if 'alert_type' in locals() and coin in self.WHITELISTED_TOKENS:
            if alert_type == 'OPEN':
        ...
        # Always save positions regardless of notification status
    ...
    def send_open_alert(self, coin, side, value, price):
        """Send OPEN position alert with whale name as clickable link (ONLY FOR WHITELISTED)"""
        if coin not in self.WHITELISTED_TOKENS:
            return
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_close_alert(self, coin, side, value, price, entry_price=None):
        """Send CLOSE position alert with whale name as clickable link (ONLY FOR WHITELISTED)"""
        if coin not in self.WHITELISTED_TOKENS:
            return
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_partial_close_alert(self, coin, side, prev_value, curr_value, reduction_pct, price):
        """Send PARTIAL CLOSE alert with whale name as clickable link (ONLY FOR WHITELISTED)"""
        if coin not in self.WHITELISTED_TOKENS:
            return
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_partial_increase_alert(self, coin, side, prev_value, curr_value, increase_pct, price):
        """Send PARTIAL INCREASE alert with whale name as clickable link (ONLY FOR WHITELISTED)"""
        if coin not in self.WHITELISTED_TOKENS:
            return
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    async def queue_fill_for_aggregation(self, trade):
        ...
        # STRICT FILTER: Ignore anything not in whitelist
        if coin not in self.WHITELISTED_TOKENS:
            return
    ...
    def show_stats(self):
        ...
        if not self.positions:
            print("\nNo BTC/ETH/SOL positions tracked")
            return
        
        print(f"\nCURRENT POSITIONS (BTC/ETH/SOL ONLY):")
        total_value = 0
        for coin, pos in self.positions.items():
        ...
        print("🎯 Tracking: BTC, ETH, SOL only")
        print()
        ...
        while self.running:
            try:
                ...
                for trade in trades:
                    ...
                    # STRICT FILTER: Skip non-whitelisted tokens entirely
                    if coin not in self.WHITELISTED_TOKENS:
                        continue
                ...
                if cycle % 10 == 0 and self.positions:
                    ...
                    print(f"POSITIONS: {len(self.positions)} BTC/ETH/SOL positions, total ${total_value:,.0f}")
                    print(f"✅ Strict filtering active (BTC/ETH/SOL only)")
                ...
                if cycle % 50 == 0:
                    ...
                    if self.pending_fills:
                        ...
                        for (coin, side), fills in self.pending_fills.items():
                            ...
                            notification_status = "📢" if coin in self.WHITELISTED_TOKENS else "🔇"
                            print(f"  {coin} {side_text}: {len(fills)} fills, ${total_value:,.0f} {notification_status}")
                        print()
    ...
    async def sync_positions_on_startup(self):
        ...
        print("\nSyncing with whale's BTC/ETH/SOL positions...")
        try:
            current_positions = await self.get_whale_positions()
            
            if not current_positions:
                print("No BTC/ETH/SOL positions found on exchange")
                return
            
            positions_found = 0
            for coin, position_data in current_positions.items():
                # Double-check whitelist (should already be filtered)
                if coin not in self.WHITELISTED_TOKENS:
                    continue
            ...
            if positions_found > 0:
                self.save_positions()
                print(f"Synced {positions_found} BTC/ETH/SOL positions")
            else:
                print("No significant BTC/ETH/SOL positions found (>$100K)")
    ...
    async def start(self):
        print(f"\nSTARTING WHALE POSITION TRACKER V2 (BTC/ETH/SOL ONLY)")
        print("=" * 50)
        ...
        print("    ☀️☀️ $1M-$9.99M")
        print("    ☀️ $100K-$999K")
        print("  Other tokens: 🔇 TRACKED BUT NO NOTIFICATIONS")
        print()
        ...
        try:
            ...
            if self.positions:
                ...
                self.send_telegram_message(
                    ...
                    f"📊 BTC/ETH/SOL positions: {len(self.positions)}\n"
                    f"💰 Total value: {self.format_value(total_value)}\n"
                    f"🐋 <a href='{coinglass_url}'>{self.whale_name}</a>"
                )

def main():
    print("WHALE POSITION TRACKER V2 - BTC/ETH/SOL NOTIFICATIONS ONLY")
    print("=" * 50)
    print("Features:")
    print("• Real-time position tracking for ALL tokens")
    print("• Telegram alerts ONLY for BTC, ETH, and SOL")
    print("• Other tokens tracked silently (no notifications)")
